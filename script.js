    ...
    convertToKg(opt, val) {
        switch (opt.value) {
            case 'kg':
                return val.value
            case 'lb':
                return +(val.value / 2.20462).toFixed(6)
            case 'gm':
                return +(val.value / 1000).toFixed(6)
            case 'mg':
                return +(val.value / 1000000).toFixed(6)
        }
    }

    // converting the result from convertToKg fn to opposite box's option and displaying 
    convertAndDisplay(convertedBox, type, value) {
        switch (type.value) {
            case 'kg':
                convertedBox.value = value
                break
            case 'lb':
                convertedBox.value = +(value * 2.20462).toFixed(6)
                break
            case 'gm':
                convertedBox.value = +(value * 1000).toFixed(6)
                break
            case 'mg':
                convertedBox.value = +(value * 1000000).toFixed(6)
                break
        }
    }
}
...
const checkOptionParent = (massFn) => {
    if(optionsParent.value === 'mass') massFn 
}

firstBox.addEventListener('keyup', function () {
    checkOptionParent(mass.convertAndDisplay(secondBox, optionsChildTwo, mass.convertToKg(optionsChildOne, firstBox)))
})

secondBox.addEventListener('keyup', function () {
    checkOptionParent(mass.convertAndDisplay(firstBox, optionsChildOne, mass.convertToKg(optionsChildTwo, secondBox)))
})
